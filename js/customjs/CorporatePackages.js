...
function generateRandomIds() {
	...
	$.ajax({
		...
		error : function(jqXHR, textStatus, errorThrown) {
			$("#myModalabout").modal('hide');
			$(".loading").hide();
			$('#i_errorGenerate').html('Ids cannot be generated. Try again later!');
...
function updateCandidateRegistrationAction(p_exam_id,p_package_id,p_status,p_count){debugger;
   if(!(navigator.onLine)){
	    toastr.error('You are offline. please check internet connection.');
		return;
   }
	...
	if($('input[name=actions'+p_package_id+'_'+p_exam_id+']:checked').val() == undefined){
...
/*	if((document.getElementById('#c_action1_'+p_package_id+'_'+p_exam_id)).checked){
	...
	l_map.registrationAction = $('input[name=actions'+p_package_id+'_'+p_exam_id+']:checked').val();
	...
	$.ajax({
		...
		success : function(data) {
			...
			$(".loading").hide();
			if (data.status == 'SUCCESS') {
			    l_html+="<button data-placement='top' title='Manage Your Candidates.' class='btn-gradient' id='check' onclick='managCredential(\""+p_exam_id+"\",\""+p_package_id+"\",\""+p_status+"\",\""+p_count+"\")'>Manage candidates</button>"; 
                $('#i_candi_reg_'+p_package_id+'_'+p_exam_id).html(l_html);
					...
					if (response.status == "SUCCESS") {
						...
						for (var i = 0; i < l_data_list.length; i++) {
							...
							if (l_map.examStatus == 'COMPLETED') {
                                l_html +="<input type='hidden' id='c"+(i + 1)+"' value='COMPLETED'>";
							...
							if (l_map.examStatus != 'COMPLETED') {
								l_html +="<input type='hidden' id='c"+(i + 1)+"' value='NOT_COMPLETED'>";
...
function checkIndividual(p_id, p_index) {debugger;
...
function bulkCandidateActions(p_action_type) {debugger;
